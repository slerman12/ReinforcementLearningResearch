...
# Vision
vision = Vision.Vision(brain=Brains.FullyConnected({"input_dim": state_space}))
...
# Experience replay
experience_replay = Memories.ExperienceReplay(capacity=, attributes=)
...
if __name__ == "__main__":
    ...
    for episode in range(100000):
        ...
        for _ in range(max_episode_length):
            ...
            # Represent
            representation = long_term_memory[0].represent(state)

            # Remember
            remembered = [memory.remember(representation, agent.perception_of_time) for memory in long_term_memory]

            # Expect TODO expect for multiple memories (5 dims)
            expectation = long_term_memory[0].expect(remembered)
            ...
            # Experience
            experience = {"state": state, "representation": remembered[action]["representation"],
                          "expected": remembered[action]["expected"],
                          "remember_concepts": remembered[action]["remember_concepts"],
                          "remember_attributes": remembered[action]["remember_attributes"], "action": action,
        ...
        # Store short term memories in long term memory, re-build KD tree, and update weights
        agent.learn()
